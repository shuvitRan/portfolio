...
function Swarm({ type ,count, mouse }) {
  ...
  const particles = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const xFactor = -10 + Math.random() * 40
      const yFactor = -10 + Math.random() * 40
      const zFactor = -10 + Math.random() * 40
  ...
  return (
    // <div style={{ width: '100%', height: '100%' }} >
      ...
      <Canvas 
        ...
        <pointLight position={[-100, -100, -100]} intensity={15} color="rgb(0, 0, 40)" />
        <Swarm type='lineh' mouse={props.mouse} count={120} />
        {/* <Swarm type='linev' mouse={props.mouse} count={20} /> */}
